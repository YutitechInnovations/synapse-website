...
export default function ResearchPaperManagement() {
  ...
  const fetchPapers = async () => {
    try {
      setIsLoading(true);
      const response = await getAllResearchPapers(100, 0);
  ...
  const handleDeletePaper = async (paper) => {
    const paperId = paper.research_paper_id || paper.id;

    if (!paperId) {
      toast.error("Research paper ID not found. Cannot delete this paper.");
      return;
    }

    const promise = new Promise(async (resolve, reject) => {
      try {
        const response = await deleteResearchPaper(paperId);

        if (response.status === "success") {
          setPapers((prevPapers) =>
            prevPapers.filter((p) => (p.research_paper_id || p.id) !== paperId)
          );
          resolve("Research paper deleted successfully!");
        } else {
          reject(
            new Error(response.message || "Failed to delete research paper")
          );
        }
      } catch (error) {
        ...
        reject(
          new Error(
            error.message ||
              "Failed to delete research paper. Please try again."
          )
        );
      }
    });
    ...
    toast.promise(promise, {
      loading: "Deleting research paper...",
    ...
    <div className="w-full bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen p-3 sm:p-4 md:p-6">
      ...
      <div className="mb-6 md:mb-8">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 lg:gap-6">
          ...
          <Link
            href="/admin/research-paper-management/create"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm md:text-base text-gray-600">
                Manage and edit your research papers with ease
              </p>
            </div>
        ...
        {isLoading ? (
          <div className="text-center py-12 md:py-20">
            ...
            <p className="text-gray-400 text-sm mt-2">
              Preparing your content management dashboard
            </p>
          </div>
        ) : error ? (
          <div className="text-center py-12 md:py-20">
            ...
            <table className="w-full hidden lg:table">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {papers.map((paper, index) => (
                  <tr
                    key={paper.research_paper_id || paper.id}
                  ...
                  >
                    ...
                    <td className="px-4 md:px-6 py-4 md:py-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/admin/research-paper-management/edit/${
                            paper.research_paper_id || paper.id
                          }`}
          ...
          <div className="text-center py-12 md:py-20">
            ...
            <p className="text-gray-600 mb-6 md:mb-8 max-w-md mx-auto px-4">
              Get started by creating your first research paper to share
