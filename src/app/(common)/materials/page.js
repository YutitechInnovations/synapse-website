...
const TABS = [
  {
    ...
    heroTitle: "Materials",
    heroSubtitle:
      "Educational resources and guides from the world of orthodontics",
    gridTitle: "Materials",
  },
];
...
function BlogsContent() {
  ...
  const [originalArticles, setOriginalArticles] = useState([]); // Keep original order
  ...
  useEffect(() => {
    ...
    if (!tabParam && activeTab !== "materials") setActiveTab("materials");
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        ...
        const response = await getAllArticles(itemsPerPage, offset);
        ...
        if (response.status === "success") {
          ...
          setOriginalArticles(cleanedArticles || []); // Store original order
          setArticles(cleanedArticles || []);

          // Use resp_count from API response for total count
          if (response.resp_count !== undefined) {
            setTotalCount(response.resp_count);
            setTotalPages(Math.ceil(response.resp_count / itemsPerPage));
          }
        } else {
  ...
  useEffect(() => {
    ...
    const sortedArticles = [...originalArticles].sort((a, b) => {
      switch (sortBy) {
        case "newest":
      ...
      <main className="w-full max-w-7xl 3xl:max-w-screen-xl mx-auto px-4 py-12 md:py-16 mt-28">
        ...
        {!isLoading && articles.length > 0 && (
          <div className="flex justify-center items-center mb-6">
            <p className="text-gray-600 text-sm">
              Page {currentPage} of {totalPages} • Showing {articles.length}{" "}
              materials
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {articles.map((article, index) => (
              <Link
                ...
                href={`/materials/${article.article_id}`}
                ...
                <div className="relative w-full h-48 overflow-hidden">
                  {article.image ? (
                    <Image
                      ...
                      onError={(e) => {
                        console.error(
                          "Public article image failed to load:",
                          article.image
                        );
