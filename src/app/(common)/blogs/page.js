    ...
    const fetchBlogs = async () => {
      try {
        ...
        const response = await instance.get(
          `/blogs/get_all_blogs?limit=${itemsPerPage}&offset=${offset}`
        );
        ...
        if (response.data.status === "success") {
          ...
          // Use resp_count for total
          if (response.data.resp_count !== undefined) {
            setTotalCount(response.data.resp_count);
            setTotalPages(Math.ceil(response.data.resp_count / itemsPerPage));
          }
        } else {
      ...
      <main className="w-full max-w-7xl 3xl:max-w-screen-xl mx-auto px-4 py-12 md:py-16 mt-28">
        ...
        {!isLoading && blogs.length > 0 && (
          <div className="flex justify-center items-center mb-6">
            <p className="text-gray-600 text-sm">
              Page {currentPage} of {totalPages} • Showing {blogs.length} blogs
            </p>
          </div>
        )}
                ...
                <div className="relative w-full h-48 overflow-hidden">
                  {blog.image ? (
                    <Image
                      ...
                      onError={(e) => {
                        console.error(
                          "Public blog image failed to load:",
                          blog.image
                        );
