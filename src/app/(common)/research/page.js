...
import { getAllResearchPapers } from "../../../services/researchPapers";
...
const TABS = [
  {
    ...
    heroSubtitle:
      "Latest research and academic insights from orthodontic science",
...
function BlogsContent() {
  ...
  const [originalPapers, setOriginalPapers] = useState([]); // Keep original order
  ...
  const itemsPerPage = 50; // Show 9 papers per page
  ...
  useEffect(() => {
    ...
    if (!tabParam && activeTab !== "research") setActiveTab("research");
  ...
  useEffect(() => {
    const fetchPapers = async () => {
      try {
        ...
        const response = await getAllResearchPapers(itemsPerPage, offset);

        if (response.status === "success") {
          // Clean any malformed image URLs
          const cleanedPapers = response.data.map((paper) => ({
            ...paper,
            image: cleanImageUrl(paper.image),
          }));
          setOriginalPapers(cleanedPapers || []); // Store original order
          setPapers(cleanedPapers || []);

          // Calculate total pages from response
          if (response.resp_count !== undefined) {
            setTotalCount(response.resp_count);
            ...
            // If no total provided, check if we got a full page
            if (cleanedPapers.length === itemsPerPage) {
            ...
            setTotalCount(
              (currentPage - 1) * itemsPerPage + cleanedPapers.length
            );
          }
        } else {
  ...
  useEffect(() => {
    if (originalPapers.length === 0) return;

    const sortedPapers = [...originalPapers].sort((a, b) => {
      switch (sortBy) {
        case "newest":
      ...
      <main className="w-full max-w-7xl 3xl:max-w-screen-xl mx-auto px-4 py-12 md:py-16 mt-28">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-12 gap-6">
          ...
          <div className="relative">
            <div className="flex items-center gap-3 bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm border border-gray-200">
              <span className="text-[#195B48] font-medium text-sm">
                Sort by:
              </span>
              <button
                ...
                {SORT_OPTIONS.map((option) => (
                  <button
                    ...
                    className={`w-full px-4 py-3 text-left flex items-center justify-between hover:bg-gray-50 transition-colors duration-200 ${
                      sortBy === option.key
                        ? "bg-[#195B48]/10 text-[#195B48] font-semibold"
                        : "text-gray-700"
                    }`}
                  >
        ...
        {!isLoading && papers.length > 0 && (
          <div className="flex justify-center items-center mb-6">
            <p className="text-gray-600 text-sm">
              Page {currentPage} of {totalPages} • Showing {papers.length}{" "}
              papers
            </p>
          </div>
        )}

        {/* Enhanced Research Papers List Grid */}
        {isLoading ? (
          <div className="text-center py-20">
            <div className="relative">
              <div className="w-16 h-16 border-4 border-[#195B48]/20 border-t-[#195B48] rounded-full animate-spin mx-auto mb-6"></div>
              <div
                className="absolute inset-0 w-16 h-16 border-4 border-transparent border-t-[#195B48]/40 rounded-full animate-spin mx-auto"
                style={{
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-3xl font-bold text-[#195B48] mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {papers.map((paper, index) => (
              <Link
                key={paper.research_paper_id}
                href={`/research/${paper.research_paper_id}`}
                ...
                <div className="relative w-full h-48 overflow-hidden">
                  {paper.image ? (
                    <Image
                      ...
                      onError={(e) => {
                        console.error(
                          "Public research paper image failed to load:",
                          paper.image
                        );
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto leading-relaxed">
              Be the first to share insights and expertise with the orthodontic
              community. Start writing and inspire others with your knowledge.
            </p>
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 max-w-sm mx-auto border border-gray-200">
              <div className="text-sm text-gray-500">
                <p className="font-medium">
                  Check back soon for new articles from our experts
                </p>
              </div>
            </div>
          </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-semibold flex items-center gap-2 transition-all duration-300 ${
                currentPage === 1
                  ...
                  : "bg-white border-2 border-[#195B48] text-[#195B48] hover:bg-[#195B48] hover:text-white shadow-md hover:shadow-lg"
              }`}
            >
              <svg
                className="w-4 h-4"
                  ...
                  return (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg font-semibold transition-all duration-300 ${
                        currentPage === page
                          ? "bg-gradient-to-r from-[#195B48] to-[#144636] text-white shadow-lg scale-105"
                          : "bg-white border-2 border-gray-300 text-gray-700 hover:border-[#195B48] hover:text-[#195B48] shadow-md"
                      }`}
                    >
                      {page}
                    </button>
