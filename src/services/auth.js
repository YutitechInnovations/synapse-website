...
export const doctorLogin = async (payload) => {
  try {
    ...
    if (error.response?.status === 400) {
      const errorMessage = error.response?.data?.message || "Invalid request. Please check your credentials.";
      throw new Error(errorMessage);
    }
    if (error.response?.status === 404) {
      const errorMessage = error.response?.data?.message || "User not found. Please check your email.";
      throw new Error(errorMessage);
    }
    const errorMessage =
